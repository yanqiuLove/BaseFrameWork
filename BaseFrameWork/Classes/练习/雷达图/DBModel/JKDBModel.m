...
- (BOOL)saveOrUpdateByColumnName:(NSString*)columnName AndColumnValue:(NSString*)columnValue
{
    ...
    if (record) {
        ...
        if ([primaryValue intValue] <= 0) {
            ...
            self.sc_pk = [primaryValue integerValue];
            return [self update];
        }
    }else{
        return [self save];
...
- (BOOL)save
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        self.sc_pk = res?[NSNumber numberWithLongLong:db.lastInsertRowId].intValue:0;
        NSLog(res?@"插入成功":@"插入失败");
    }];
    return res;
}
...
+ (BOOL)saveObjects:(NSArray *)array
{
    ...
    [jkDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (JKDBModel *model in array) {
            ...
            model.sc_pk = flag?[NSNumber numberWithLongLong:db.lastInsertRowId].intValue:0;
