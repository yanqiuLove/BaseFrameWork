...
- (void)drawBody {
    ...
    CGContextSetRGBFillColor(context, 238.0 ,238.0 ,238.0 ,1);
...
- (void)buildPart {
    ...
    for (int j = 0; j<r; j++) {
        ...
        // 添加百分比
        UILabel *partLabels = [self buildPartLabelWithText:[NSString stringWithFormat:@"%d",(j-5)*(-1)] Frame:CGRectMake(center_w-LENGTH(30), center_h -radar_l + radar_l*j/r-LENGTH(4), LENGTH(25), LENGTH(6))];
...
- (void)drawRect:(CGRect)rect {
    ...
    // 画百分比占比
    [self buildPercent];
    // 需要再画一次主体内容  解决主体部分的分割线不清晰问题
    [self drawBody];
}

//设置外环的填充色
//    CGContextSetRGBFillColor(context, arc4random() % 100 / 100.0 ,arc4random() % 100 / 100.0 ,arc4random() % 100 / 100.0 ,0.2);
//    CGContextDrawPath(context, kCGPathFillStroke);
//画虚线
//    CGFloat lengths[] = {5, 5};
